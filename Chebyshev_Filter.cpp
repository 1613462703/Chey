...
void Chebyshev_Filter::Filter_Data(double pre_filt_signal[], int size, double filt_signal[], std::vector<std::vector<double>> coeff_filt)
{
    // 初始化 filt_signal 数组
    ...
    std::vector<double> temp_v(size, 0.0); // 预分配 size 长度
...
//--------------------------------------------------------------------------------------------------//
//--------------------------------------------------------------------------------------------------//
std::vector<std::vector<double>> Chebyshev_Filter::Calculate_Chebyshev(double f1, double f2, double sf, double Rp, int order_filt, int type_filt) {
...
void Chebyshev_Filter::Cheby1(double Input_Array[], int Number_of_Elements, double Sampling_Frequency, double Cutoff_Frequency, double Ripple_db, int Order, double Output_Array[], int type_filt) {
    std::vector<std::vector<double> > coeff_final(2);

    CH_FI::Chebyshev_Filter ch_fi;
    coeff_final = ch_fi.Calculate_Chebyshev(Cutoff_Frequency, 0, Sampling_Frequency, Ripple_db, Order, type_filt);
    ch_fi.Filter_Data(Input_Array, Number_of_Elements, Output_Array, coeff_final);
}
void Chebyshev_Filter::Cheby1band(double Input_Array[], int Number_of_Elements, double Sampling_Frequency, double Lower_Cutoff_Freq, double Upper_Cutoff_Freq, double Ripple_db, int Order, double Output_Array[], int type_filt) {
    std::vector<std::vector<double> > coeff_final(2);

    CH_FI::Chebyshev_Filter ch_fi;
    coeff_final = ch_fi.Calculate_Chebyshev(Lower_Cutoff_Freq, Upper_Cutoff_Freq, Sampling_Frequency, Ripple_db, Order, type_filt);
    ch_fi.Filter_Data(Input_Array, Number_of_Elements, Output_Array, coeff_final);
}